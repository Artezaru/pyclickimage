...
latexpdf:
	...
	cd $(BUILDDIR)/latex && pdflatex pyclickimage.tex && pdflatex pyclickimage.tex
...
app:
	echo "from pyclickimage.__main__ import __main_gui__" > run_gui.py
	echo "__main_gui__()" >> run_gui.py
	pyinstaller --name pyclickimage --onefile --windowed run_gui.py
	rm run_gui.py
	rm -rf build
	rm run_gui.spec
